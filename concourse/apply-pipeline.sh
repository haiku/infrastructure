...
for ARCH in $ARCHES; do
	...
	if [ "$TEAM" == "continuous" ]; then
		...
		$FLY_CLI -t haiku set-pipeline -n -p $BRANCH-$ARCH -v branch=$BRANCH -v arch=$ARCH -l $SECRETS -v profile=$PROFILE -v media=$MEDIA -v bucket=$BUCKET -y days=\[$DAYS\] -c pipelines/haiku-release.yml
	fi
